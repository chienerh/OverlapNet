...
def generateSiameseNetworkBoth(input_shape=(50, 50, 1), config={}, smallNet=False):
  ...
  # The overlap head
  head_method = getattr(sys.modules[__name__], 'generate' + config['overlap_head'])
  prediction_overlap, prediction_function_angle = head_method(encoded_l, encoded_r, config)
  # prediction=generateDeltaLayerReLuNetworkHead(encoded_l, encoded_r)
  ...
  # Generate a keras model out of the input and output tensors
  siamese_net = Model(inputs=[left_input, right_input], outputs=[prediction_overlap, prediction_function_angle, prediction_orientation])
  return siamese_net
